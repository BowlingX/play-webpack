...
object WebpackPlugin extends AutoPlugin {
  ...
  object Format extends Enumeration {
    val NESTED, PLAIN = Value
  }

  object autoImport {
    lazy val webpackManifest = settingKey[Seq[File]]("JSON file that has been generated by the webpack assets plugin.")

    lazy val webpackManifestFormat = settingKey[Format.Value]("Format of the manifest file")
  }
  ...
  def buildInfoScopedSettings(conf: Configuration): Seq[Def.Setting[_]] = inConfig(conf)(Seq(
    ...
    sourceGenerators in Compile += task[Seq[File]] {
      val file = (sourceManaged in Compile).value / "com" / "bowlingx" / "webpack" / "Manifest.scala"
      val manifest = webpackManifest.value
      val format = webpackManifestFormat.value
  ...
  def buildInfoDefaultSettings: Seq[Setting[_]] = Seq(
    ...
    webpackManifestFormat := Format.NESTED
  )
