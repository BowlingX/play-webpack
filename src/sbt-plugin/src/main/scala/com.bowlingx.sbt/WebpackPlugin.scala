...
object WebpackPlugin extends AutoPlugin {
  ...
  object autoImport {
    lazy val webpackManifest: SettingKey[Option[File]] = SettingKey[Option[File]](
      "webpackManifest",
  ...
  def buildInfoScopedSettings(conf: Configuration): Seq[Def.Setting[_]] = inConfig(conf)(Seq(
    watchSources ++= webpackManifest.value.map(Seq(_)).getOrElse(Seq.empty),
    sourceGenerators in Compile += task[Seq[File]] {
      val file = (sourceManaged in Compile).value / "com" / "bowlingx" / "webpack" / "Manifest.scala"
      val manifest = webpackManifest.value
      manifest.map( manifestFile => {
        val code = ManifestCompiler(manifestFile).generate()
        IO write(file, code)
        Seq(file)
      }) getOrElse Seq.empty
    }
